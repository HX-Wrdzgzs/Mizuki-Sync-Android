...
修改 main.py 和 lxns_update.py 中的配置项：填入你申请的落雪 LX_CLIENT_ID, LX_CLIENT_SECRET, DEV_API_KEY。确认数据缓存目录 SONG_DB_PATH 的读写权限。启动后台清洗守护进程：Bashnohup python3 lxns_update.py > update.log 2>&1 &
启动 FastAPI 服务：Bashuvicorn main:app --host 0.0.0.0 --port 8000
客户端编译 (Android)使用 Android Studio 打开项目。确保项目级别的 build.gradle 已配置 Kotlin 插件。全局搜索 https://api.mizuki.top，将其替换为你自己部署的后端域名或 IP。Sync Project & Build APK。📸 界面预览(提示：建议在这里放几张 App 截图，例如主页、搜索页筛选菜单、详情页)主页概览曲库搜索与筛选乐曲详情<img src="docs/home.png" width="250"><img src="docs/search.png" width="250"><img src="docs/detail.png" width="250">📜 声明与致谢本项目的数据均来源于 落雪查分器 (LXNS)，感谢落雪团队提供稳定强大的 API 支持。游戏资产（曲绘、图标等）版权归原游戏开发商 (SEGA) 所有，本项目仅作学习交流及个人数据统计使用。
